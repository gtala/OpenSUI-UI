    ...
    <div className="flex min-h-screen flex-col items-center justify-between p-24">
        <div className="w-full max-w-6xl items-start justify-between font-mono text-sm lg:flex lg:py-20">
          <div className="w-1/3 bg-black lg:relative">
            {metadata[0]?.content?.fields.image_url &&
                   <img
                   src={
                     "https://ipfs.io/ipfs/" +
                     metadata[0].content?.fields.image_url.replace("ipfs://", "")
                   }
                   alt="Nft Img"
          ...
          <div className="w-2/3 mx-20">
            <h2 className="text-3xl">{metadataTest.fields.name} #1</h2>
            <h4 className="text-lg mt-4 mb-8">{metadataTest.fields.description}</h4>
            <div className="border-[0.5px] border-gray-600 rounded-md p-4">
              <p className="py-4">Traits:</p>
              <div className="flex flex-wrap -mx-2">
                {metadataTest.fields.attributes_keys.map(
                  (key: string, index: number) => (
                    ...
                    >
                      ...
                      <p>{metadataTest.fields.attributes_values[index]}</p>
                    </div>
                  )
                )}
              </div>
